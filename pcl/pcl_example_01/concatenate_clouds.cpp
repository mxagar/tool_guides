/* Simple program in which pointcouds are concatenated.
 * Look at:
...
int main (int argc, char** argv) {
  ...
  // Two concatenations shown:
  // 1. A pointcloud is appended to another (-p)
  //    For that to work, both need to have the same point definitions (types and fields)
  // 2. Pointclouds with different sets of fields are merged (-f)
  //    For that to work, both point clouds need to have the same amount of points
  if (strcmp(argv[1], "-p") == 0)
  {
    cloud_c  = cloud_a;
    cloud_c += cloud_b;
